...
Path/URI Parameters
Path method can be used to extend the Base URL to various endpoints
Lets add Path to BASEURL
rest.Base("https://api.example.com/").Path("param1")
User can add as many paths as possible by chaining them one after other.
...
Query
User can attach Query Paramters to our Request URL.
